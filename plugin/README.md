...
- **A tiny stdio bridge** (`bin/rembric-bridge.mjs`, ~80 LOC) that reads `.rembric-slug` from the project root and path-scopes the MCP URL to `/mcp/<slug>` so the Rembric server pins the correct project on connect. No agent-side `project.use` call, no router-fallback codepath.
...
```bash
claude plugin marketplace add /Users/jesus.mejias/Desktop/rembric
...
Drop a one-line `.rembric-slug` file in each project's root containing the slug:

```bash
echo "my-app-slug" > .rembric-slug
```

That's it. The bridge reads it on every MCP session start and builds the URL `${server_url}/mcp/my-app-slug`. The Rembric server pins the project from the URL path automatically.

If `.rembric-slug` is missing or contains an invalid slug, the bridge falls back to path-less `/mcp` and writes a diagnostic to stderr (visible in `claude --debug`). The session still works — the agent operates in global scope until something else pins a project.

### Whether to commit `.rembric-slug`
...
Pick something stable and human-readable:

| Project type                  | Suggested slug                      | Notes                                  |
| ----------------------------- | ----------------------------------- | -------------------------------------- |
| GitHub repo `acme/foo`        | `acme-foo`                          | Disambiguates from other `foo` repos   |
| Internal product `my-app`     | `my-app`                            | Single name when collision risk is low |
| Notes / scratch / docs folder | `notes-personal`, `docs-team`       | Add a qualifier                        |
| Monorepo with subprojects     | `acme-foo-frontend`, `acme-foo-api` | One slug per subdir                    |

The slug must match `^[a-z0-9]([a-z0-9-]{0,62}[a-z0-9])?$`.

## Token budget

Always-on cost (per turn, on top of MCP tool listings):

```
4 command listings      ≤ 40 tok
─────────────────────────────────
Total                   ≤ 40 tok
```

On-invoke cost (paid only when components fire):
...
The proactive-save protocol is delivered via the MCP `initialize.instructions` (~500 chars, paid once per MCP connection by every client) so it does not show up in the plugin's per-turn budget.

Verify the always-on cost any time:

```bash
claude plugin details rembric
```

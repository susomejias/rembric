...
- **Design tokens locked**: brutalist dark theme, lime accent, self-hosted fonts only. Changing tokens requires an OpenSpec change against `dashboard` spec. CSS lives in `apps/web/src/app/globals.css` — never inline `<style>`.

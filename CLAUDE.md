...
- **Per-component versioning (six components, `node-workspace` cascade, NO linked-versions group).** release-please runs six components: `server` (`apps/server`); **`plugin-shared`** (`apps/plugin`, `exclude-paths` = ALL four client dirs) which owns ONLY the shared assets (`bin/`+`hooks/`+`commands/`+`scripts/`+`README.md`), package name `@rembric/plugin`; `claude-code-plugin` (`.claude-plugin/`, own `package.json` depending on `@rembric/plugin`, `extra-files: ["plugin.json"]`); `codex-plugin` (`.codex-plugin/`, same shape); `opencode-plugin` (`.opencode-plugin/`, `release-type: simple`); `hermes-plugin` (`.hermes-plugin/`, `release-type: simple`). The `node-workspace` plugin (`merge: false`) cascades a `+patch` to `claude-code-plugin` and `codex-plugin` whenever `@rembric/plugin` bumps (they bundle the marketplace-cached bridge), each in its OWN separate version-titled PR — so Claude/Codex keep independent versions/tags/CHANGELOGs but still refresh on shared changes. A Claude-only change bumps only Claude. opencode/hermes are `simple` (no `package.json`) so node-workspace skips them; they re-fetch shared assets from `main` at install time. **Every `node` component MUST have a `package.json`** (node-workspace reads all node packages to build the graph). There is **no `linked-versions` group** (its grouped PR title lacks `${version}` and never auto-tagged). The client `package.json`s are release-graph nodes only — NOT pnpm workspace members (the `apps/*` glob doesn't match nested dirs). Tags: `server-v*`, `plugin-shared-v*`, `claude-code-plugin-v*`, `codex-plugin-v*`, `opencode-plugin-v*`, `hermes-plugin-v*`.

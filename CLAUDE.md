...
| Dev server       | `pnpm run dev` (Next.js dev server in `apps/web`; `docs/docker.md`)          |
...
Single Node process, single SQLite file. `apps/web` is the Next.js App Router app that owns every HTTP surface — `/mcp`, `/api`, `/healthz`, OAuth and the dashboard; the extracted packages own the rest — `packages/db` is the SQL boundary (schema, migrations, repositories, client, migrate, diagnostics, query tokenizer, scope), `packages/core` holds services, consolidation, embeddings and summary, and `packages/mcp` is the protocol domain (tool definitions, server factory, scope resolution). Shared plugin tree at `apps/plugin/` ships to FIVE clients (Claude Code, Codex CLI, Hermes Agent, opencode, Pi) — see [Plugin development](#plugin-development-discipline). Pi is the odd one out: it has no built-in MCP client (intentionally, per its own docs), so `.pi-plugin/` speaks Streamable HTTP MCP itself and discovers its tools with `tools/list` instead of enumerating them. Monorepo uses pnpm workspaces with `apps/*` (deliverables) and `packages/*` (shared libraries — `db` and `core` carry the extracted code, `config` is dev-only tooling, `ui` is a scaffold with no dependents). Turborepo owns `build`, `typecheck` and `test` via `turbo.json`; `lint`, `format:check`, `test:coverage`, `eval` and `dev` stay direct scripts.

### Load-bearing invariants (do NOT violate without an OpenSpec change)

- **Append-only memory.** Rows never `DELETE`d (narrow purge exceptions in `packages/core/src/services/{memory,agent-sessions}.ts` and `apps/web/src/scripts/seed-dev.ts`, allow-listed in `apps/web/src/test/invariants.test.ts`). `content` never `UPDATE`d. Lifecycle = `status` flips (`active` → `superseded` | `archived`) plus `replaces` links. Every consolidation op journaled in `consolidation_ops`, reversible.
- **Scope enforced at service layer.** Every `MemoryService` query filters by `Scope`. Cross-scope reads return `not_found`. New MCP tools that need project scope MUST obtain it from `resolveEffectiveScope` (`packages/mcp/src/_shared.ts`), which consults both `ctx.project` (URL slug) and `SessionRouter` (`project.use` pins, roots discovery) — never read `ctx.project` in isolation, and never construct the default project's scope in a handler.
...
- **Review state is derived, never stored.** Two orthogonal staleness axes: _decay_ (access + confidence, keyed on `last_seen_at` — the sweep archives) and _review_ (affirmation, keyed on `max(created_at, last confirmation) + per-type TTL` in `packages/core/src/services/review.ts`). `reviewState`/`needsReview` are computed at read time in `MemoryService`/repository reads — no column, no sweep, no new mutation verb. Re-affirming a `needs_review` memory is the existing `memory.confirm` (append-only event); reading it (`last_seen_at`) deliberately does NOT clear it.

Path-scoping contract (in `packages/mcp/src/_shared.ts::resolveEffectiveScope`): every connection resolves to exactly one project and no tool argument can name another. `/mcp/<slug>` is fixed to that project, and a slug naming no project is refused with `project_not_found` rather than falling back. `/mcp` resolves through the `SessionRouter` entry (a `project.use` pin or roots discovery) and otherwise the `is_default` project. `scope_locked` refuses a _switch_, never a scope. Tool input schemas are strict, so an unknown property is refused rather than silently stripped.

### Data access pattern

- ALL SQL (Drizzle builder or raw) lives under `packages/db/src/`: one repository per aggregate in `packages/db/src/repositories/`, DB-level introspection (PRAGMA/dbstat/VACUUM/ping) in `packages/db/src/diagnostics.ts`, schema and migrations in `packages/db/src/{schema,migrations}/`. Services orchestrate (scope resolution, validation, transactions); dashboard handlers call `admin*` repository reads + service mutations. No SQL in services/dashboard/mcp/server — grep-enforced by `invariants.test.ts` (data-access confinement).
...
- **Timestamps go through the `<Time>` component** (`apps/web/src/components/dashboard/time.tsx`). It emits `<time data-rembric-ts>` with a UTC fallback and re-renders in the viewer's timezone after mount. Never hand-write `toISOString` / `toLocaleString` in views.
- **Destructive actions MUST use the `ConfirmSubmit` dialog** (`apps/web/src/components/dashboard/confirm-submit.tsx`), which ties a Radix `AlertDialog` to its form. Pass `tone="danger"` for irreversible, `"warn"` for reversible. Call sites to mirror live under `apps/web/src/app/dashboard/`.
...
- **Design identity (redesigned; formal spec pending)**: dark monochrome zinc surfaces — `#09090b` bg, `#101012` card, `#18181b` raised, `#1f1f23` 1px borders, `#fafafa`/`#a1a1aa`/`#71717a` text — with lime `#c6f24e` as the ONLY accent (primary buttons lime fill + near-black text; chrome-only on light surfaces, never lime text on light backgrounds). Fonts: Geist (UI + display) and Geist Mono (labels/meta/data), self-hosted woff2 in `apps/web/src/app/fonts/`. Radius 16 (cards) / 12 (inputs) / 10 (buttons); soft shadows and radial lime glows instead of hard borders. List views follow the Spectrum UI data-table pattern. Changing tokens requires an OpenSpec change against the `dashboard` spec; the formal spec update is pending on `feat/dashboard-identity-redesign`, so until it lands the LIVE reference is that branch code (`sessions-table.tsx`, `command-bar.tsx`, `globals.css`). CSS lives in `apps/web/src/app/globals.css` — never inline `<style>`.
...
- **`.agents/skills/npm-security-best-practices/`** — before adding any dependency or editing `package.json` / `.npmrc` / `pnpm-workspace.yaml` / lockfile / CI install / Dockerfile install layers. The repo enforces default-deny lifecycle scripts (`.npmrc::ignore-scripts=true`) with per-package exceptions enumerated only in `pnpm-workspace.yaml::allowBuilds` (which also carries non-granting `false` entries that must still be justified — e.g. `unrs-resolver`, a transitive of `eslint-plugin-import-x` whose postinstall the repo denies), pinned by `ALLOWED_BUILD_SCRIPTS` (`grep -rn ALLOWED_BUILD_SCRIPTS`) — adding a `true` entry reds `pnpm test` until the inventory is edited too, and so does a `pnpm rebuild` argument in the Dockerfile that the inventory does not pin. For the rest of the posture (`blockExoticSubdeps`, `minimumReleaseAge`, what `--frozen-lockfile` actually guarantees) read `CONTRIBUTING.md#adding-a-dependency` — do not restate it here.
- **`.agents/skills/rembric-plugin-development/`** — before touching anything under `apps/plugin/`. Covers the five clients, per-client gotchas (`references/per-client-gotchas.md`), and the mandatory e2e validation (`references/e2e-walkthrough.md`).

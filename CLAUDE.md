...
- **Per-component versioning (four components, NO linked-versions group).** release-please runs four components: `server` (`apps/server`); **`plugin-shared`** (`apps/plugin`, `exclude-paths` = only `.opencode-plugin` + `.hermes-plugin`) which owns the shared assets (`bin/`+`hooks/`+`commands/`+`scripts/`+`README.md`) **and** the Claude Code + Codex surfaces — the two clients' `plugin.json` versions are `extra-files` of it (`.claude-plugin/plugin.json`, `.codex-plugin/plugin.json`), so any commit touching shared assets or either client dir bumps all three in lock-step; `opencode-plugin` (`.opencode-plugin/`, independent); `hermes-plugin` (`.hermes-plugin/`, independent). opencode and hermes are independent because their installers re-fetch shared assets from `main` at install time (they bundle nothing). There is **no `linked-versions` group** — it was removed because release-please hardcodes the grouped PR title with no `${version}`, so grouped release PRs could never auto-tag on merge. Each release PR now carries a version in its title and tags itself. Tags: `server-v*`, `plugin-shared-v*`, `opencode-plugin-v*`, `hermes-plugin-v*` (the pre-merge `claude-code-plugin-v*` / `codex-plugin-v*` tags are frozen history).

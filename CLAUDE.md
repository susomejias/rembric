...
Rembric ships one plugin tree (`plugin/`) consumed by four agent marketplaces today (Claude Code, Codex CLI, Hermes Agent, OpenClaw) and is designed to extend to others (future Cursor/Windsurf/etc.). To keep cross-client support sustainable:

- **Shared logic lives in the HTTP API contract.** The anchor is `src/server/api-router.ts` (capability `http-api`). Per-client adapters MAY be written in any language — bash for the shell-hook clients (Claude Code, Codex CLI), Python for Hermes Agent's `MemoryProvider`, and plain ESM JavaScript for OpenClaw's native plugin are siblings, NOT duplicates. They implement the same set of POSTs against the same endpoints and SHALL stay in lock-step on payload shape and failure semantics. Shell-only assets (`plugin/scripts/`, `plugin/bin/`) remain shared across the shell-hook clients via `${CLAUDE_PLUGIN_ROOT}`; the Hermes provider lives at `plugin/.hermes-plugin/` and reads the same API contract via Python `urllib`; the OpenClaw plugin lives at `plugin/.openclaw-plugin/` and reads the same API contract via Node `fetch`.
- **Per-client divergence ONLY when the platform forces it.** Two near-duplicate file pairs and one whole-cloth Python tree diverge today, all because the platforms have different supported syntax/event sets — not for cosmetic reasons:
  ...
  - **OpenClaw plugin tree**: `plugin/.openclaw-plugin/{openclaw.plugin.json, package.json, plugin.mjs, mcp-client.mjs, http-client.mjs, tools.mjs, hooks.mjs, memory-capability.mjs, commands.mjs, *.test.mjs}`. Whole-cloth different from the shell-hook clients — OpenClaw runs native plugins in-process inside its Node gateway via the `OpenClawPluginApi`, not via shell subprocesses. The plugin claims OpenClaw's `kind: "memory"` slot, registers tools / hooks / interactive matcher / memory capability programmatically (no JSON hook config — Claude/Codex `hooks.json` files are detect-only under OpenClaw), and talks to the same `/api/<slug>/sessions(*)` lifecycle endpoints AND speaks Rembric's MCP wire protocol over HTTP to call memory tools. Plain ESM only — the OpenClaw plugin SDK (`@openclaw/plugin-sdk`) is a `workspace:*` package upstream and not installable outside the OpenClaw monorepo. The third-party install path is `openclaw plugins install git:<repo-url>` (sources accepted by OpenClaw's CLI are `path | archive | npm-spec | git:repo | clawhub:pkg` per `auto-reply/reply/plugins-commands.ts:46`); there is NO local `marketplace.json` at the repo root for OpenClaw, unlike Claude/Codex.
...
### Releasing a new plugin version — MUST bump `version` in ALL FOUR manifests

`plugin/.claude-plugin/plugin.json`, `plugin/.codex-plugin/plugin.json`, `plugin/.hermes-plugin/plugin.yaml`, AND `plugin/.openclaw-plugin/openclaw.plugin.json` declare a `version` field (plus the mirror in `plugin/.openclaw-plugin/package.json` which the manifest-invariants test pins to match). Claude Code uses the field as the cache key for `/plugin update` (official docs: `code.claude.com/docs/en/plugins-reference#version-management`) and Codex stores plugins under `~/.codex/plugins/cache/<marketplace>/<plugin>/<version>/`. Hermes does not cache by version (the plugin ships via `plugin/.hermes-plugin/install.sh`, idempotent re-fetch), and OpenClaw does not cache by version when installing via `git:repo` either (each `openclaw plugins install` re-fetches), but keeping their `version:` in lock-step keeps `plugin/CHANGELOG.md` and operator-facing diagnostics meaningful. If you ship changes WITHOUT bumping the version, the two shell-hook clients will keep serving the cached old code and `/plugin update` will report "already at the latest version" — users have to manually uninstall + reinstall to recover.

**Rule**: any time you change something in `plugin/` that users need to see (scripts, hooks, mcp.json, bin/, the Hermes provider, the OpenClaw plugin, manifests themselves), bump ALL FOUR manifest versions in the same commit. Use SemVer:

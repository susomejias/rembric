...
- **Hook env vars come from `${user_config.*}` substitution in the manifest, NOT from `mcp.json:env`.** Claude Code hooks run as sibling subprocesses to the MCP bridge and inherit the shell of `claude`, not the bridge's env. To bridge that gap, each hook entry in `plugin/hooks/hooks.json` (and `hooks.codex.json`) prefixes the script invocation with `REMBRIC_SERVER_URL='${user_config.server_url}' REMBRIC_API_TOKEN='${user_config.api_token}'`. Claude Code substitutes those tokens with the keychain values at fire time, so the user only enters them once (in the install wizard) and the hooks see them automatically — no shell exports required. When they're missing entirely, `_api.sh` writes a `[rembric] missing REMBRIC_*` diagnostic to stderr and exits 0. Codex's plugin engine supports the same substitution syntax — see the same pattern in `hooks.codex.json`.

...
- **Hook env vars are NOT inherited from `mcp.json:env`.** Claude Code hooks run as sibling subprocesses to the MCP bridge and inherit the shell environment that launched `claude`, not the bridge's. `REMBRIC_SERVER_URL` and `REMBRIC_API_TOKEN` must be `export`ed in the user's shell rc (`~/.zshrc` etc.) for the hooks to reach the HTTP API; the same values the plugin's `userConfig` keychain stores. Without the shell export the hooks fail silently — `_api.sh` writes a `[rembric] missing REMBRIC_*` diagnostic to stderr and exits 0. Documented in `plugin/README.md`.

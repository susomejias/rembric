...
**The extension also declares its identity on the MCP transport.** On the first `before_agent_start` — once the session row exists — it pins the transport to its own row with an id-targeted `memory.session_resume` over MCP, re-declares whenever the MCP transport re-initialises, retries a `not_found` on the next turn and stays silent if it cannot declare. The pin is what makes the model's defensive `memory.session_start` adopt the host's session instead of minting a parallel one, and what keeps concurrent sessions on one project from stepping on each other: the server resolves a declaring transport by its pin before any lookup.

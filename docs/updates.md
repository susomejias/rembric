...
- **A widened page can be _smaller_ than the narrow one, and can replace its rows entirely.** There is deliberately no home-project preference — if you widen because the answer is not in your project, home rows always winning would defeat the point — and the relevance gate is computed over the widened pool. So one strongly matching foreign row can cut home rows that were passing on their own. Measured on a real two-project corpus: a query returning **6 rows from the current project narrow returned 1 row from the other project widened**, flagged `gateShortened: true`. **That flag covers only the gate, not the other route to the same surprise:** where foreign rows outrank yours the page comes back full, no gate ran, and nothing is flagged — read `searchedProjects[]` and the rows' own projects instead. It is working as designed, but "I widened and got fewer of my own rows back" is the predictable question and this is the answer.
- **It is not free: 2.2–3.0× a narrow search, end to end**, measured through the real search path at 1 000 / 20 000 / 50 000 memories on a realistically skewed corpus, widening from the project holding most of your memories. Widening from a small one reaches 12×, because every authorized project contributes its own full candidate window — that is what stops a widened search from quietly returning fewer of your own rows than a narrow one — and a small project's narrow search is very cheap to begin with. In absolute terms the widened search stays under 140 ms at 50 000 memories. Per explicit question that is the trade the feature is. Per turn, by habit, it is a tax — which is why the tool's own description tells the agent to widen only on an explicit ask or a genuinely broad exploration.

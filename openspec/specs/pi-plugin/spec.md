...
model-initiated `memory.session_start` on the extension's own transport has to _infer_
...
path that can _insert_ a row when the resolution is ambiguous, so using it to declare an

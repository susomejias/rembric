...
All table definitions SHALL live as TypeScript files under `packages/db/src/schema/`. Migrations SHALL be generated by `drizzle-kit generate` and applied by the server on startup. Hand-written raw-SQL migrations SHALL be permitted for FTS5 and sqlite-vec virtual-table setup; these SHALL live in `packages/db/src/migrations/` alongside the generated migrations.

#### Scenario: A schema field is renamed

- **WHEN** a contributor renames a column in `packages/db/src/schema/memory.ts`
...
- **WHEN** a code reviewer inspects `packages/db/src/schema/memory.ts`
...
#### Scenario: An image built from the wrong stage of `apps/web/Dockerfile` is blocked

- **GIVEN** a regression that causes `docker-publish.yml` to produce an image that does not start the web server — a mutated `CMD ["sh", "-c", "... seed-dev.ts --reset && exec tsx watch ..."]`, or a dropped web override that falls back to `apps/web/Dockerfile`
...
- **WHEN** a code reviewer inspects `packages/db/src/schema/prompts.ts`
...
The triggers SHALL be installed by a dedicated migration `packages/db/src/migrations/000X_prompts_fts.sql`. The migration SHALL also backfill `prompts_fts` from existing rows so the index is complete on first boot after this change.
...
The migration runner at `packages/db/src/migrate.ts` SHALL execute each pending migration with the following structure:
...
- **GIVEN** the test suite in `apps/web/src/test/invariants.test.ts`
- **WHEN** CI runs
...
This table has no corresponding Drizzle schema file — like the virtual `memory_vec` table, it is queried exclusively via raw SQL from `packages/db/src/repositories/memory-repository.ts`.
...
- **THEN** it SHALL recreate `confirmations_memory_verdict_ts_idx`, and the exact-set index assertion in `packages/db/src/schema/schema-drift.test.ts` SHALL fail if it does not
...
Adding the index means `packages/db/src/schema/schema-drift.test.ts`'s pinned index set must gain it in the same commit; its going red first is the proof the index landed.

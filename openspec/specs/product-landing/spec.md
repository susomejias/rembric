...
The static marketing site for `rembric.dev`: a no-framework, no-SPA product landing authored as plain HTML + CSS + vanilla JS under `apps/landing`. Defines its authoring model (copy the dashboard design tokens rather than share a package), its `esbuild` minify build to `dist/`, its exclusion from the release-please tracks, and its Cloudflare Pages static distribution.

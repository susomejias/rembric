...
- **WHEN** the dev container's boot chain runs `tsx src/scripts/seed-dev.ts --reset` (invoked automatically as part of `pnpm run dev:docker:up`)
...
- **WHEN** `tsx src/scripts/seed-dev.ts --reset` is invoked (from any context — manual `docker exec`, prod container by accident, a misrouted CI job)
...
pnpm run build:css \
  ...
  && tsx src/scripts/seed-dev.ts --reset \
...
- **THEN** the dev container SHALL build `dist/dashboard/public/...`, run `seed-dev.ts --reset` (the env gate is satisfied; wipe is a no-op on empty tables), and start `tsx watch` in that order
...
- Pass `target: runtime` to the `docker/build-push-action@v5` step explicitly, regardless of Dockerfile stage order.
- After `docker push` but BEFORE retagging `:latest` (and any version/major aliases), run a smoke-test step that pulls the freshly-pushed immutable `:sha-<short>` tag and inspects its image config. The smoke test SHALL apply **three independent assertions**, ANY of which can fail the workflow:
  ...
  - **Image size check**: pull the image's compressed size from the registry manifest (`docker buildx imagetools inspect --raw <image>` summed across layers, or equivalent) and fail if the total exceeds 600 MB. The dev stage produces a >1 GB image; the runtime stage is ~300 MB. The threshold catches wrong-stage publishes even when the other two signals fail.
...
The Dockerfile SHALL be structured so that:

- The `runtime` stage is the **last** `FROM ... AS <name>` declaration. This makes `docker build .` (without `--target`) produce the runtime image by default — fail-safe against future edits that drop `target: runtime` from the publish workflow.
- The `runtime` stage SHALL declare `LABEL rembric.stage=runtime`.
- The `dev` stage SHALL declare `LABEL rembric.stage=dev` (purely diagnostic; helps anyone inspecting a pulled image to identify which stage they have).

This catches Dockerfile-level regressions (broken `COPY` paths, missing dependencies, native-module compile failures, dev-deps drift) before they reach a release publish, AND prevents the dev stage from being shipped as the canonical image.
...
- **THEN** the step SHALL invoke `docker/build-push-action@v5` with `target: runtime`
...
- **GIVEN** a faulty Dockerfile change has been merged that shifts the `runtime` stage so the `dev` stage ends up being the last (and `target: runtime` was also accidentally removed from the workflow)
...
- **AND** the workflow's error output SHALL clearly identify which forbidden substring was found

#### Scenario: Image label check catches a wrong-stage publish independently

- **GIVEN** a faulty build where the published image's `Cmd` was rewritten (e.g., `tsx watch` → `node --watch src/...`) such that the substring check no longer matches, but the image is still built from a stage that lacks `rembric.stage=runtime`
...
- **GIVEN** a faulty publish that produces a >1 GB image (containing dev deps + source tree + tsx) regardless of what `Config.Cmd`/`Labels` say

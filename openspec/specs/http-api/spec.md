...
For each provided field, the server SHALL apply the precedence rule. **Winning the precedence contest and replacing the stored column are two different outcomes, and conflating them is what made the previous wording false for `summary`:**

- If `final` in body is `true`: the write WINS precedence (last-final-wins) and the `*_final` column is set to `true`. What then lands in the column depends on the field. `title` is REPLACED outright. `summary` is replaced outright too, EXCEPT against a row that already carries a curated summary (`summary_final = true` and `summary IS NOT NULL`), where the write is MERGED section-wise instead: the `##` sections it carries replace their stored counterparts and a section it omits keeps its stored text (`sessions`, "A curated session-summary write MUST be merged section-wise with the stored summary"). Measured through this endpoint: storing `## Goal\nShip X\n## Files\nsrc/a.ts` and then writing `## Goal\nShip Y` leaves `## Goal\nShip Y\n## Files\nsrc/a.ts` — the omitted `## Files` survives. Two controls pass in the same run: a headingless `A` followed by `B` still stores `B` (with no heading on either side the merge degenerates to replacement), and `title` still replaces.
- If `final` in body is `false` (or omitted) AND the row's `*_final` column is `false`: write the field, leave `*_final` unchanged.
- If `final` in body is `false` AND the row's `*_final` column is `true`: silently skip writing this field — no merge and no replacement. Measured in the same run: the stored summary is byte-identical afterwards.

...
This requirement governs the widening argument only, and therefore cannot constrain a connection whose _base_ scope is already global. A path slug that does not resolve to an existing project currently resolves to the global scope rather than to an error, so on such a connection every read is a global read and ignoring `include_global` changes nothing. That fallback is a separate defect in scope resolution, tracked outside this requirement; until it is fixed, the isolation guarantees above hold for a path-scoped connection whose slug resolves.

...
- **THEN** the `InitializeResult.instructions` SHALL contain the same protocol triggers (including the session-close protocol sentence and the `2000`-char cap) and a note indicating the connection is global-scope and that project memories require opening `/mcp/<slug>` or sending `X-Rembric-Project`

...
- The script SHALL emit the generic nudge `[rembric] If this is a continuation of recent work, call memory.context before responding.` Slug-specific path-scoping is handled by the bridge at MCP connect time — the script does not duplicate that resolution.

...
`proposal.md` carries the motivation and the measurements. This document records the decisions by number (D1–D22, plus D2a, cited from `tasks.md`), the arithmetic that has to come out right, and the questions deliberately left for the owner.
...
A defect with no observable symptom and no self-healing path is the one kind that must be pinned by a test that fails without the fix. Task 3.6 asserts `count(*) FROM memory_vec WHERE partition_key = '__global__'` is 0 **and** `count(*) FROM memory_vec > 0` (the second half is the non-vacuity control — CLAUDE.md's before/after digest rule), plus a dense kNN in the default partition returning > 0; task 12.2 deletes the vec loop and requires it to go red.
...
- [Risk] **The `memory_vec` repointing is forgotten or half-done, and nothing reports it** → Mitigation: D4's failure chain is measured link by link; task 3.6's assertion has a non-vacuity control (`count(*) FROM memory_vec > 0`, not just `= 0` for the global partition); task 2 exercises recall **end-to-end through `searchWithAbstention`**, not `knnByQueryVector` directly, with a control row native to the project that must also return; task 12.2 mutation-deletes the vec loop and requires red.
...
9. **`memory_vec`: DELETE the ex-global rows and re-INSERT them carrying the same blob** with `partition_key = <new project id>` (D4). Never `UPDATE`.

**Measured result** against a copy of the real corpus enriched to 16 global rows: **51 → 51** memory rows, 18/18 assertions, `foreign_key_check` **0** violations, `integrity_check` **ok**, per-table deltas conserved. No table rebuild.

**First boot after upgrade.** Migration runs inside `migrate()` at `db/client.ts:72`. **There is no migration logging at all today** — `migrate()` returns `{applied, skipped}` and `client.ts:72` discards the result — so this change creates that surface. `printBootstrapBanner` (`server/bootstrap.ts:487`, called at `:386`) is the natural home. Given the change's justification is truthfulness about where memories live, moving rows silently is not acceptable: **log the chosen slug and the repointed count**. The entity rebuild then drains in the background (D15).

...
`SUMMARY_MAX_CHARS` SHALL be set high enough to carry a rich handoff summary (the design records the chosen value). The constant SHALL remain the single source of truth, exported and imported by the MCP zod schema (`apps/server/src/mcp/session-tools.ts`) and by the HTTP-layer truncation helper, so no layer can drift from the service-level cap. No client SHALL maintain its own, larger bound on what it sends: a client-side wire bound SHALL be derived from this constant, so a payload cannot be selected against one budget and then re-cut against a smaller one.
...
Truncation SHALL happen at most once for a given payload. A client that must bound its own payload SHALL truncate on the same side, and SHALL prefer cutting at a record boundary over cutting mid-record, so the persisted artefact remains parseable.

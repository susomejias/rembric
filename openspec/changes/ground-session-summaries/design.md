...
**D4 — `Stop` gains a second, synchronous entry that may block exactly once, and fails open.**
The existing `stop-sync.sh` entry stays `async: true` — it is a pure side effect and must not delay the turn. A new non-async entry performs one bounded request asking whether the session already has a curated summary. If it does, or if the session has nothing worth summarising, the hook exits 0 silently. If it does not, the hook emits `{"decision":"block","reason":…}` and the reason carries the long rubric plus the extracted facts from D3, so the model summarises against evidence rather than recollection.

The fail-open rule is absolute and is the reason this is safe to ship: any non-2xx, any timeout, any parse failure, any missing configuration exits 0 with no output. A memory server is an optional accessory to the host; it must never be able to prevent an agent from finishing. The at-most-once rule uses the loop-guard flag the host supplies in the `Stop` payload for exactly this purpose — **the flag's exact name is to be confirmed against the host contract during implementation, not assumed**, and if it proves absent the gate ships as `additionalContext` (non-blocking feedback) rather than as a block.

Alternative considered: _block on `SessionEnd` instead_. Rejected — `SessionEnd` fires when the session is already closing, so there is no model turn left to act on the feedback. That is why the current `SessionEnd` wiring can only write, never ask.
...
**D6 — `SubagentStop` contributes to the same extraction.**
Delegated work is currently invisible: a subagent's edits and commands never appear in the parent's summary, so a session that did all of its work through agents can look empty. `SubagentStop` appends the subagent's extracted facts to the parent session's skeleton. It does not block — a subagent finishing is not the moment a handoff is owed.

## Risks / Trade-offs

- [Risk] A synchronous `Stop` hook adds latency to every turn's end → Mitigation: one request with a short timeout, and the check is a single indexed read. Budget the added latency and record it; if it exceeds the budget the gate becomes `additionalContext` on the async entry instead.
- [Risk] Blocking `Stop` makes the agent do work the user did not ask for → Mitigation: at most once per session, and never when the session has nothing worth summarising. The block is also the only mechanism that turns a documented obligation into one that is actually met, which is the point.
- [Risk] Fail-open means the gate silently does nothing when the server is unreachable → Accepted deliberately, and it is the correct direction: the failure mode of a missing reminder is a thinner summary, and the failure mode of a trapped agent is a broken session.
...
- **Does the host's `Stop` payload carry a loop-guard flag, and under what name?** Decides whether D4 ships as a block or as non-blocking feedback. To be answered by reading the host contract during implementation, not assumed.

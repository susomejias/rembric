...
- **`Stop` gains a synchronous gate that asks once, and only when it would help.** Split from the existing async sync: a second, non-async `Stop` entry checks whether the session already carries a curated summary and, if not, returns `decision: "block"` with a reason. The reason is where the _detailed_ rubric lives, because it has no length budget and arrives at the one moment the model can still act on it. Three non-negotiables: **fail-open** on any error, timeout or non-2xx (a server that is down or slow MUST NOT trap the agent), **at most one block per session**, and no block when the session has nothing worth summarising.
- **The rubric gets one source of truth.** It is currently six copies, already divergent. The enriched rubric — work done, decisions and their reasons, what was verified and how, what was left unfinished, files — is defined once and every client surface derives from it, with a fixture asserting they agree. The MCP tool description is exempt from carrying the long form: it is hard-capped by `DESCRIPTION_MAX_LENGTH = 1900` against the host's truncation ceiling, so it keeps a terse pointer and the long form travels in the `Stop` reason.
- **`SubagentStop` is wired, because delegated work is currently invisible.** A subagent's file edits and commands never reach the parent session's summary. Its contribution feeds the same extracted skeleton.

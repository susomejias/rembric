...
**Workspace membership diverges from the `.pi-plugin/` precedent, deliberately.** `pi-plugin/spec.md:23` accepts being outside `pnpm-workspace.yaml::packages` — unlinted, unreached by `pnpm -r` — and the sentence that makes it acceptable is that the extension "SHALL therefore declare **no runtime dependencies**". This package cannot make that trade: its single dependency must be resolved by the lockfile and integrity-verified under `--frozen-lockfile`, and its recovery logic needs executable tests with mutation gates. The Docker image is unaffected — both install lines filter with `--filter @rembric/server...` (`apps/server/Dockerfile:26,95`).
...
### D12 — One runtime dependency, exact-pinned, with the cost on the record

`@modelcontextprotocol/sdk` is the only runtime dependency. Justification per `supply-chain-hygiene/spec.md:279`: it is the protocol's reference implementation, this repo **already ships it** (`apps/server` declares `^1.0.0`, lockfile-resolved to `1.29.0`), and the alternative is owning the wire protocol ourselves — the surface `mcp-remote` demonstrably got wrong.

**The cost, measured 2026-08-15** (`npm install --ignore-scripts` into an empty directory): `mcp-remote@0.1.38` → **80 packages, 7.0 MB**; `@modelcontextprotocol/sdk@1.30.0` → **97 packages, 24 MB**. The SDK ships its server-side halves (`express`, `hono`, `cors`, `jose`, `pkce-challenge`, `express-rate-limit`) in `dependencies`, so consumers install them even though only the client transport is imported. Accepted knowingly, with a gate task to measure the end-to-end cold-start delta so the number is published rather than guessed. Partly offset, and measured at the same gate: the four pre-MCP HTTP requests and the OAuth callback port disappear, and one node process per session disappears with the wrapper.

The dependency is declared at an **exact** version, not a range: a caret range would let a new upstream SDK release change behaviour on a user's machine without any Rembric release — the hole the pin exists to close, one level down.

**Alternative rejected: bundle the SDK and publish zero runtime dependencies.** It would shrink the install and satisfy the zero-default outright, and it is what `mcp-remote` does — which is exactly why confirming its `404` behaviour required reading `chunk-65X3S4HB.js` instead of a dependency tree. A consumer's advisory tooling would no longer see the SDK version, so a CVE in it would be invisible to them.
...
- **[Trade-off] Install weight grows from 80 packages / 7.0 MB to 97 packages / 24 MB** (D12) → **Accepted because** the alternatives are bundling (which destroys the consumer's view of the dependency) or owning the wire protocol (the failure being fixed). Partly offset by four fewer HTTP requests and one fewer process per session; both sides measured at the gate.
...
3. **Which exact `@modelcontextprotocol/sdk` version is pinned?** The lockfile resolves `1.29.0`; the measurements used `1.30.0`. The applier picks one that satisfies `minimumReleaseAge` at the time and dedupes with `apps/server`'s resolution, and records the number in `tasks.md`.

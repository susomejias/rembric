...
- [ ] 12.6 Record as **rejected**: bundling the SDK to reach zero runtime dependencies (D12) — it hides the dependency from a consumer's advisory tooling. Rejected: pinning at N−1 to close the tag-versus-publish window (D11) — it permanently guarantees we ship code other than what we just released. Rejected: accepting an optional positional URL override (D7) — a second way to express the endpoint that diverges the moment one changes. Rejected: having `install.sh` rewrite an existing `opencode.json` (D6) — a much larger promise than this change should make. Rejected: the package names `@rembric/mcp-bridge` (too vague) and `@rembric/mcp-bridge` (does not say what it bridges).

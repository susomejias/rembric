...
### Requirement: Ceremony runs MUST be inspectable and reversible from the dashboard

The dashboard SHALL list ceremony runs at `/dashboard/ceremony` and SHALL show per-run details at `/dashboard/ceremony/:id` including each op with its LLM reasoning. Each op SHALL have an "Undo" action; each run SHALL have an "Undo entire run" action.
...
- **WHEN** the operator visits `/dashboard/ceremony`
...
- **WHEN** a contributor clones the repo and runs `npm install`

...
- **Entity recall hints at turn START via a dedicated endpoint.** A new lightweight `POST /api/<slug>/sessions/:id/recall-hints` endpoint accepts `{prompt}` (already `<private>`-redacted client-side, server processes at most the first 500 chars), extracts entities, matches the entity index, and returns `{lines: string[]}`. Read-only and process-and-discard: the prompt is never persisted (append-only invariant). Clients call this synchronously before the model responds — pi via `before_agent_start`, opencode via `chat.message`, bash via `prompt-search.sh` — and merge the returned lines into the model's context so they are visible from the first token. The turn body stays `{usedTools}` (+title) exactly as today.
...
- `plugin-session-protocol`: Client call sites invoke the hints endpoint at turn start (pi `before_agent_start`, opencode `chat.message`, bash `prompt-search.sh`). The turn-report body retains only `{usedTools}` (+title); the `prompt` field is no longer part of the turn report.
...
- `apps/plugin/scripts/prompt-search.sh` — calls the hints endpoint synchronously and echoes returned lines before the model responds.

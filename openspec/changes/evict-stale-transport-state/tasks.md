...
- [ ] 7.1 Read `.agents/skills/rembric-plugin-development/SKILL.md` and `references/per-client-gotchas.md` before touching `apps/plugin/`. Sanity check afterwards: `git ls-files apps/plugin/` still shows one copy of every shared resource; this change must touch `.pi-plugin/index.ts` only.
- [ ] 7.2 In `apps/plugin/.pi-plugin/index.ts`, on a `404` to a request that carried an `mcp-session-id`: discard the id, re-run `initialize` plus the initialized notification, retry the original request once. `404` only — `401`, `403`, `429` and `5xx` keep throwing as they do today. No loop, no tool re-registration.
- [ ] 7.3 Extend `apps/plugin/.pi-plugin/plugin.test.ts` with arms against its stub server (the shape at `:1139` already exists): recovery succeeds and sends exactly one extra `initialize`; a second `404` surfaces as an error with no third attempt; a `401` triggers no `initialize`; no extra tool registration occurs.
- [ ] 7.4 Mutation: make the retry unconditional on status → the `401` arm must go red. Make the retry a loop → the second-failure arm must go red.
- [ ] 7.5 **Operator step, real edge.** Follow `.agents/skills/rembric-plugin-development/references/e2e-walkthrough.md` against `pnpm run dev:docker:up`, drive a real Pi session, force the server to evict its transport (temporarily shrink the threshold or evict by hand through a test hook — do not ship the hook), and confirm the next tool call succeeds. Record the observed re-initialize count.
- [ ] 7.6 **Operator step, and a merge gate (design Open Question 2).** Same forced eviction against a real Claude Code connection on the dev stack: does the host re-initialize, and does the user see anything? Record the result verbatim. If it does not recover, do not widen anything silently — apply the recorded fallback (keep the router half plus the orphan clause, defer the transport half) and say so in the PR.
- [ ] 7.7 Record Codex CLI, opencode and Hermes as **unverified** unless actually driven, with one line each on why. An unverified client described as fine is the failure mode this repo has already paid for.

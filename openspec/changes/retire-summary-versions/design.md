...
- `measure-summary-clobbering` phase 1 reads versions 14 and 15 of session `01a00320-1fae-7360-862f-0ae866b38803` out of the operator's live deployment. That change already names the dependency in its own `tasks.md:4-5` ("its source is `session_summary_versions` — a table a follow-on change retires"). The capture must happen before this change reaches that deployment.
...
### D6. The end-of-turn reminder's "no suppression" rule keeps its primary argument and loses its secondary one
...
Migration number is assigned at apply time. `measure-summary-clobbering` also claims `0034`; whichever change lands second takes the next free number and updates the two filename fixtures in `migrations.test.ts`.
...
- **Does the residual in-section risk warrant an instrument before this lands?** Deliberately left open rather than defaulted. `measure-summary-clobbering` is that instrument and is a separate change with its own evidence bar; this change neither requires it nor blocks it, and the owner has accepted the risk without it.

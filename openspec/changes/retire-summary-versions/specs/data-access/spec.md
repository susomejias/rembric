...
The `agent-sessions` aggregate owns exactly one table. It briefly owned a second, `session_summary_versions`, whose membership was justified by a write-ordering constraint — the version row was appended in the same transaction as the `UPDATE` it recorded. That table is retired (`sessions`, "Every curated session-summary write MUST append a version row in the same transaction" is REMOVED), the constraint went with it, and the aggregate's repository SHALL NOT carry a summary-version method of any name.

...
- Adding a third retrieval stream. The audit found published evidence that a graph stream *reduced* Recall@5 versus BM25 alone in its own author's benchmark; any new stream must earn its place through the harness first.
...
- **Token containment is not BM25.** It ignores IDF, so a match on common words scores higher than a relevance ranker would like. Acceptable: it is used for the *reported* similarity and for ordering inside an already-relevance-ordered pool, not as the retriever.

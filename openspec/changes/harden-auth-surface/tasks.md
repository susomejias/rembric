...
- [x] 5.4 Smoked against `pnpm run dev:docker:up` (dev stack, seeded): async-auth `/healthz` 200 + MCP `initialize` on `/mcp/demo` 200; body limit `413` (5 MB POST); login cookie has no `Secure` on http-loopback + invalid login `401`; pre-auth lockout — 9×`401` → `429` with `Retry-After: 60`, and a valid bearer is `429` while the IP is locked. (OAuth project-binding is disabled in the dev stack without `REMBRIC_PUBLIC_URL`; it stays covered by the in-process `oauth-e2e` / `oauth` tests. Note: the dev image's own healthcheck is broken independently — it execs `/nodejs/bin/node`, absent in the `dev` target — so the container reads `unhealthy` while fully serving; pre-existing, unrelated.)

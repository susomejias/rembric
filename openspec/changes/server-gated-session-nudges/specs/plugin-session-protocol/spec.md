...
**Reason**: Every load-bearing clause of this requirement is superseded, and three of them by their own admission.

Its cadence clause bound the reminder to "the same per-session turn counter the start-of-turn nudge already uses"; `session-nudges` removes that counter from every client and gates on server state instead. Its content clause required the payload to carry "the grounded facts extracted from the session" — a call measured at ~0.5 s of `jq` per firing on a synchronous host path, whose motivation was a model rewriting ten to twenty turns from recollection, and which the section-wise merge landed by `refine-session-summary-writes` retires by letting the model describe only the recent stretch it can still see. Its "SHALL NOT consult the server" clause names its own successor verbatim: _"A follow-up MAY add a read endpoint and narrow this; until it does, no requirement here SHALL assert the reminder consults server state."_ `POST /api/<slug>/sessions/:id/turn` is that endpoint.
...
The `<private>` redaction helper, the truncation helpers, the stderr diagnostic, the session HTTP client, the transcript accumulator, the summary-body builder, the flush helpers, the session-end call, the session-resume call, **the turn-report call and the per-session cache of the server's returned lines**, and the texts of the lines that remain client-composed, SHALL exist in exactly one JS/TS implementation, at `apps/plugin/bin/rembric-plugin-core.mjs`. Every JS/TS client SHALL import them from there and SHALL declare no local copy.

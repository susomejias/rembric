...
- **AND** `last_work_at`, `last_activity_at` and `last_nudge_at` SHALL all have advanced
...
- **AND** `last_activity_at` SHALL have advanced while `last_work_at` and `last_nudge_at` SHALL be unchanged

...
- `bring_up` (`apps/plugin/install.sh`) creates `./data` (if missing) immediately before `docker compose up -d`, then makes it accessible to the container's non-root `UID 10001`: first tries `chown 10001:10001 ./data` (the precise, `docs/docker.md`-sanctioned fix — only root/`CAP_CHOWN` can do this). If that fails, it checks `./data`'s current owner before deciding anything is actually wrong — a `chown` failure alone just means "not root," not "broken" — and only falls back to `chmod 0777` (with an explicit, non-suppressed warning) when the owner genuinely isn't uid 10001 yet. This runs unconditionally right before an actual bring-up (not merely offered), self-healing an already-broken `./data` from a pre-fix crash loop, while never touching (or warning about) a `./data` that was already correctly owned.

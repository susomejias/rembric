...
- Relaxing append-only. `last_activity_at` is a new mutable _session_ column, in the same class as `summary` and `deleted_at`; memory `content`/`title` remain frozen and nothing is deleted.

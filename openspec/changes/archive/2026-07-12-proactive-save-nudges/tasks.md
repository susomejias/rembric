...
- [x] 7.4 Plugin bumped to `0.18.0` via release-please (PR #238, merged), no `server` image rebuild triggered — confirmed via `git tag`/`package.json` and the release PR's own gating on `server_release_created`.
- [x] 7.5 Cross-client e2e — verified live today across all three clients, against the operator's real hosted server (not `dev:docker:up`, which wasn't available at authoring time; the live server gave a stronger signal since it carries real load-bearing data): **cadence mechanics work correctly on every client.** `prompt-nudge.sh` (Claude Code + Codex) and the opencode/Hermes equivalents all fire the save nudge every 5 turns and the summary nudge on turn 1 + every 10, confirmed by direct code inspection, unit tests (including new Hermes stderr diagnostics added today specifically to make this observable), and live sessions. Turn latency is unaffected (injection-only, no blocking I/O added).
      **New finding, not anticipated at proposal time:** the _model's compliance_ with the nudge is unreliable in low-signal conversations. Observed on Hermes, opencode, and Claude Code independently today: a synthetic, low-content session (15+ turns, nothing substantively worth summarizing) received the turn-1 and turn-10 summary nudges correctly but the model never acted on them — sessions stayed RAW (`summary_final=0`) well past both trigger points. This is a nudge-strength/model-adherence gap, not a cadence bug; tuning N would not fix it (the nudges already fired on schedule). Left as an open question for a possible future change (e.g. escalating nudge urgency past a turn threshold, mirroring the existing `_SAVE_HINT_URGENT` pattern) — out of scope to fix within this change.

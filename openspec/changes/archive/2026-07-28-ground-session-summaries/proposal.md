...
- **Truncation keeps the tail, in one place, against one constant.** The end of a session carries the conclusions; the beginning carries the setup. `truncateSummary` becomes tail-keeping so the server stops discarding what the plugin deliberately selected, and — **corrected during implementation, see `tasks.md` 1.3** — the plugin's wire bound deliberately stays ABOVE the server cap, because a client cannot know a given server's cap at runtime. What was wrong was the DIRECTION, not the size: two tail-cuts are idempotent, a tail-cut followed by a head-cut is what produced a middle window. The cut is a character slice, not a record boundary. The truncation marker moves to the front (`…[truncated]` prefix), because a reader must be able to tell a whole summary from the tail of a long one.
...
- **Delegated work stays invisible for now, and that is recorded rather than specified.** A subagent's edits never reach the parent's summary — measured: the parent transcript carries no subagent turns at all. Fixing it needs the facts to reach the PARENT's record, which `POST /summary` cannot do because it overwrites. Deferred to its own change, with the requirement removed from the delta so no published text claims behaviour that does not exist.
...
- `plugin-session-protocol` — the end-of-turn reminder, its cadence, and its fail-open contract.
- `claude-code-plugin` — the ninth handler entry, the two `Stop` entries' opposite execution models, and the narrowing of the decision that once declined `Stop` for model-facing output.
- `codex-distribution` — the eighth handler entry and the second `Stop` script.

Delegated work (`SubagentStop`) is **deferred to its own change and its requirement was pulled from this delta rather than shipped unimplemented**: the subagent's facts have to reach the PARENT's record, `POST /summary` overwrites rather than appends, and the alternatives are new HTTP surface or a per-session statefile whose lifecycle nothing owns.

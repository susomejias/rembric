...
The existing `stop-sync.sh` entry stays asynchronous — it is a pure side effect and must not delay the turn. A new non-async entry decides from the transcript and the turn counter alone — no request, see the correction below — and when a summary is owed emits the host's non-interrupting feedback carrying the long rubric plus the extracted facts from D3.
...
The terse section list lives in one exported TypeScript constant. **Correction from review**: the long form does NOT live beside it — a TypeScript constant for it had zero importers while the text that shipped was hand-written a second time in the end-of-turn hook, so the long form now lives in `apps/plugin/test/nudge-fixtures.json` with the other cross-language nudge text and the hook is held to it there. Every client surface — the MCP instructions block, the Hermes provider, the opencode plugin, `post-compact.sh` — carries the same text, and a test enumerates the call sites and fails when one drifts. This is the same lock-step failure the plugin discipline already names for shared resources: six copies of one string with no fixture is how five of them came to disagree with the sixth.

**D6 — DEFERRED, and this decision was NOT shipped.** It read as a shipped decision ("it appends the subagent's extracted facts to the parent session's record") while `proposal.md` recorded the same work as deferred and the requirement was pulled from the delta. Nothing in `apps/plugin/` wires a subagent-completion event. The gap it describes is real and measured — the parent transcript carries no subagent turns — but the facts must reach the PARENT's record and `POST /summary` overwrites rather than appends, so it needs either new HTTP surface or a per-session statefile. Its own change.

## Risks / Trade-offs

- [Risk] A synchronous end-of-turn hook adds latency to every turn's end → **Mitigation as SHIPPED, which is not what this line first described**: there is no request at all. The gate is a local counter file, and extraction is skipped entirely on non-firing turns and above a 32 MiB transcript ceiling — added because jq over a 213 MB transcript measured 22.6 s on this blocking path.
- [Risk] An end-of-turn reminder costs tokens on every turn it fires → Mitigation: it fires at the existing counter's cadence, not every turn, never when the session has nothing worth summarising, and — added during review — never once the transcript shows `memory.session_summary` was already called.

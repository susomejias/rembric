...
- **Add a CI invariant test** at `apps/server/src/test/invariants.test.ts` that fails if any tracked file contains the legacy install URL prefix (the `…/main/plugin/` form under `raw.githubusercontent.com/susomejias/rembric/` and `github.com/susomejias/rembric/blob/`). The three `openspec/specs/{open-source-distribution,hermes-agent-plugin,opencode-plugin}/spec.md` files are explicitly allow-listed (they document the 404 contract and MUST keep the legacy URL verbatim), the test file is self-allow-listed (it owns the rule), and `openspec/changes/**` is excluded by path (active and archived change dirs are work-in-progress docs).

...
| opencode    | per-session flag set in `message.part.updated` when `part.type` is `tool`                                                     | **verified** — the installed SDK's `Part` union names `tool` alongside `reasoning`, `file`, `snapshot`, `patch`, `agent`, `retry` and the step markers, so the flag is pinned to `tool` rather than to "not `text`"      |
...
| Hermes      | the LAST TURN of `sync_turn`'s `messages` kwarg — role outside `{user, assistant, system}` OR an assistant `tool_calls` field | **code path traced end to end in upstream source at a pinned commit; not observed at runtime** — see the split below                                                                                                     |

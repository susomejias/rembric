...
**Decision: Make the session-summary nudge more active via wording only (no hook, no plugin change).**
The summary nudge was passive because its trigger was the literal `before saying "done"` — a phrase the model evades simply by not using the word. Rebinding the trigger to `before ending any turn with real work` makes the condition fire on essentially every working turn, which is far harder to evade, at roughly char-neutral cost (adjacent clauses are compacted to fit the 800-char cap). The lever is deliberately the standard MCP `initialize.instructions` block only — reaching Claude+Codex server-side — NOT a `UserPromptSubmit`/Stop hook or any per-client plugin edit. Alternatives considered: (a) a Stop/SessionEnd hook that detects "work happened but no summary saved" and forces a re-summary — strongest enforcement but requires touching every client's hook plumbing, rejected for the "stay standard, don't get in the middle of each client" constraint; (b) a `UserPromptSubmit` injection — same per-client cost, rejected; (c) imperative intensifiers ("ALWAYS/MUST") without changing the trigger — rejected as cosmetic, the evasion is the trigger not the tone. opencode/Hermes do not consume `instructions`, so this nudge is a Claude+Codex improvement; their summary coverage already has plugin-side fallbacks (dispose-time transcript flush) out of scope here.

...
For *new* slugs created after this change (via `project.use({slug, autocreate: true})` or via the dashboard), the value SHALL match the strict regex `^[a-z0-9]([a-z0-9-]{0,62}[a-z0-9])?$`. Paths SHALL NOT be persisted in the projects table and SHALL NOT be accepted by any tool argument.
...
When neither mechanism has fired, the connection SHALL be in *global* scope. The auto-detection via MCP `roots` MAY upgrade the scope on `/mcp` connections from global to a project, but only by activating an *existing* slug (see the next requirement).
...
**Reason for removal**: replaced by the modified scope-resolution requirement above. The `X-Rembric-Project` header is a v0.1-era footgun: with slug-as-identity and a tools-based switching protocol, the header is redundant *and* impossible to make safe (no way to ask "did you mean to switch?" against a header value). Pre-1.0 versioning permits the breaking change.

**Migration**: agents currently sending `X-Rembric-Project` must reconfigure to (a) connect at `/mcp/<slug>` directly, or (b) call `project.use({slug})` after `initialize`. The release notes call out the required reconfiguration.

### Requirement: Projects MUST be uniquely identified by their absolute path

**Reason for removal**: replaced by the slug-only identity requirement above. Path-based identity collapses two checkouts of the same logical project into separate rows, breaks cross-machine reuse, and lets filesystem layout pollute the model. The slug is the cross-machine logical identifier.

**Migration**: the migration that renames `projects.path` → `projects.slug` is forward-only and value-preserving. No data loss; legacy slugs continue to function read/write under their original (looser) regex.

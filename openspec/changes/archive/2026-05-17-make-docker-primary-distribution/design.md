...
- Binds `${REMBRIC_PORT:-8787}:8787` (all interfaces) by default. **Revised 2026-05-17 after first-operator feedback:** the original design had `127.0.0.1:` for "safe-by-default" loopback, but the LXC + plugin-on-laptop topology IS the canonical case for this project, and forcing every selfhost operator to drop an override file as their first step was bad UX. The bearer token is the real security boundary; stricter operators opt into loopback via a `docker-compose.override.yml` with `ports: !override - '127.0.0.1:${REMBRIC_PORT:-8787}:8787'`. Matches the industry default for selfhosted compose (Linuxserver, Plex, Home Assistant, Frigate).

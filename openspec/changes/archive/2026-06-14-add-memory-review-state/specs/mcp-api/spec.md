...
- **AND** `needsReview` SHALL contain at most 5 `active` in-scope memories whose derived `reviewState = 'needs_review'` (see the `memory` capability), ordered oldest `reviewBaseline` first, each entry carrying `{ id, type, snippet, reviewAfter, ageMs }` (where `ageMs = now - reviewBaseline`, the time since last affirmation) so the agent can re-affirm with `memory.confirm`, supersede with `memory.save` + `topic_key`, or — when it contradicts another memory — fall through to the existing `memory.judge` flow, without further reads

...
**And it is free rather than merely cheap, which is the answer to "is it worth keeping at all".** A total order over the fused list is needed regardless of any project preference: without one, exactly-tied rows come out in whatever order the sort happens to produce, which is unreproducible across runs and makes a test over a tied pair flaky for a reason that reads as a retrieval bug. Since a secondary sort key is being paid for anyway, spending it on the home project first costs nothing. There is no cheaper variant to fall back to — dropping the preference does not remove the comparator, it only makes it say less. The expensive variant is the boost multiplier above, and that is already rejected.

**The final key is the fused position, NOT `id` — a correction to this decision, measured during phase 4.** This text originally specified "home project, then `id`" as the total order. `ulid()` is **not monotonic within a millisecond**: measured, **9 843 of 19 596** same-millisecond pairs invert relative to insertion order. An `id` tiebreak would therefore order exactly-tied rows by how a corpus happened to be written, which is arbitrary rather than stable, and it made a pre-existing test flaky on two rows tying at `0.03577472236911688`. The fused position satisfies this decision's own stated criterion — a defined order for every pair, reproducible across runs — and leaves the narrow path byte-identical. `Array.prototype.sort` is stable per ECMA-262 and the position is read from the sort's own input, so the term is provably a no-op on order; it is kept because dropping it returns 0 for two tied FOREIGN rows, which is the partial tiebreak this decision forbids. No test claims to cover a no-op.
...
The measured cost makes this concrete rather than stylistic, and the figure to quote is the **end-to-end** one, not the isolated statement: on a realistically skewed corpus a widened `memory.search` costs **2.2–3.0×** a narrow one from the project holding most of the corpus, and up to 12× from a thin one (`measurements/widened-pool-window.md` §4, instrument I2 on the shipped path, six process runs per cell; the 1.3–2.6× this decision first quoted was instrument I3, whose overlay rationed one candidate window across the union and so priced a pool that does not grow — see D20). Per explicit question that is fine. Per session start it is not. **The ≈8× this decision originally quoted is instrument I1 on a uniformly-split synthetic fixture and must not be repeated as a user-facing cost** — it is the statement-versus-end-to-end conflation `CLAUDE.md` names, and it overstates what a caller waits for by roughly threefold.
...
### D19 — The grep gate forbids the symptom, and no cheap type closes the mechanism (measured, including two failed fixes)

D4 says the widened arm is "constructed at exactly one site", enforced by the compiler first and a grep gate second. Review measured that claim and it is **approximately true rather than true**, and the two obvious repairs do not repair it. Recorded here rather than left as a comfortable assumption, because the repo's own GHSA was a widening whose legitimacy no layer could verify.

**The gap, executed.** `test/invariants.test.ts` counts occurrences of the literal `'authorized-projects'` per file. A module that already holds a `SearchScope` can derive a **wider** one without writing that literal at all:

```ts
return { ...given, projectIds: [...given.projectIds, ...extraIds] };   // no literal, type-checks
return Object.assign({}, given, { projectIds: [...] });                // likewise
```

`tsc --noEmit --strict` accepts both, so the per-file count is unchanged and the gate passes. The compiler barrier does not fire either: it refuses a widened value handed to a **write**, and this is a read.

**Failed fix 1 — a `unique symbol` brand on the arm.** Measured: the literal-from-scratch form becomes `TS2741`, which is the half everyone expects. **Spread and `Object.assign` still compile**, because a spread copies the brand property along with everything else. A brand therefore buys the appearance of nominality while leaving the actual hole open, which is worse than no fix — it would end this discussion with the hole still there.

**Failed fix 2 — a class with a `private` field.** Measured: spread now errors (`TS2741` on the private member), so it is strictly better than the brand. **`Object.assign` still compiles**, because its return type is an intersection TypeScript accepts as assignable.

**What would actually close it** is making `projectIds` unreachable from outside — an opaque value exposing methods rather than the array — which is a different shape for the type, not a guard bolted onto this one. That is a change of its own, and it should be weighed against what the gap is really worth: exploiting it requires someone writing that derivation **inside this repo**, so the two lines exist against an accidental second construction site, not against an attacker.

**What is true today, stated without varnish:** a second construction site that spells the literal is caught by the grep; one that derives from an existing widened value is caught by nothing. Task 4.5's mutation evidence exercised only literal-bearing forgeries, so its four green runs are consistent with the gap rather than evidence against it — a gate that counts a literal only proves what can be written with that literal.

### D20 — A widened search REPLACED the home project's candidates instead of adding to them (defect, measured; three decisions corrected)

Both ranked branches drew the **narrow** candidate window over the **union**, so adding an authorized project subtracted from what the home project contributed. `memory/spec.md` had required the opposite in terms — "so the pool grows with the set rather than being rationed across it", and "adding a further authorized project to the set SHALL NOT reduce what the home project contributes" — so this is a defect against the delta spec, not a design change. Reproduced at 90 identical rows per project with the foreign rows written first: the home project contributed **64 candidates narrow and 0 widened**, both pools non-empty. Full record in `measurements/widened-pool-window.md`.

**It was worse than a ranking wobble because the page came back FULL.** `gateShortened` cannot fire on a full page, and the three operator-facing documents presented that flag as the signal for a shrunken widened page, so the guidance was wrong about both cause and detectability. Those documents now separate the two mechanisms: the gate cuts rows and flags it; ranking displacement keeps the page full and flags nothing.

**Three of this change's own conclusions are corrected by the fix, and the reason each survived is the same one.**

1. **The end-to-end cost is not 1.3–2.6×** (D13, D15's risk, both docs). That figure is instrument I3 SHADOW, whose overlay carried the defect — a single `LIMIT` over the union — so it priced a pool that does not grow with the set. On the shipped path it is **2.2–3.0×** from a dominant home and up to **12×** from a thin one.
2. **The eval could not have caught it.** Its corpus is 70 memories across three projects, so no project reaches the 64-row window and the union fits inside one. Every metric is byte-identical before and after the fix — which is evidence the fix is inert at that scale, not evidence it is inert.
3. **Task 4.5's test named the property but measured the layer where it already held.** It called `vectors.knnByQueryVector` directly, and `k` is per named partition there, so it passed with the truncation present and absent. The property belongs to the candidates handed to fusion; that is where it is now pinned, and mutation-proved on both branches.

**The window function is the shape, and the obvious alternative was measured rather than assumed.** One statement per project merged by BM25 rank in JS — the shape the dense branch already uses — is 2.5× worse at 50 000 rows over seven projects, because each project pays its own full FTS `MATCH` scan; it scales linearly with width where `ROW_NUMBER() OVER (PARTITION BY project_id)` is flat. It wins exactly one measured cell (two projects at 20 000 rows, by 15%), which is recorded rather than smoothed away because two projects is the common width.

**Not changed, because its own spec requires the current behaviour.** The entity branch bounds the union with `LIMIT` and orders it `created_at DESC, id DESC`, so widening can displace home rows there too. That is `memory-entities/spec.md`'s explicit requirement — "The completeness bound SHALL remain a bound on the RESPONSE, not per project", justified by a CI-asserted ceiling on the annotation payload — and the ranked branches' scenario is scoped to "the candidates handed to fusion", which that branch has none of. The defect at that site was in `README.md`, which asserted widening it "only adds rows"; the prose is corrected, the code is not.

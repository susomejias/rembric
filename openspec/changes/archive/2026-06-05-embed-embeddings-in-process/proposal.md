...
- Replace the HTTP `EmbeddingWorker` + `LlmClient` with an in-process embedder loaded eagerly at boot (~1.1 s from the baked image; a broken model fails the boot — fail fast, no degraded mode), `pooling: 'cls'`, `normalize: true`. `memory.save` embeds the new row inline (`embedNow`, ~15 ms) so vec candidate detection actually has a self-vector — previously `source: 'vec'` could never fire at save time (pre-existing flaw, see design D7/D2).

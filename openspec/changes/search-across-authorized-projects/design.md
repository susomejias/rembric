...
So the IDF weighting a widened query applies is byte-identical to the one a narrow query applies. **A whole class of feared problem does not arise**: no document-frequency denominator changes, no term's weight moves, and the relevance level (`RELATIVE_LEVEL_RATIO = 0.4`, `hybrid-search.ts:65`) is computed against the same distribution it was calibrated on in `archive/2026-08-03-weight-relevance-levels-by-idf`. Stated as a decision rather than left implicit, because "widening changes the corpus so the IDF must be re-derived" is the obvious wrong inference and it would have cost a re-calibration sweep.
...
**Rejected, with its reason, because it is the alternative a reader will reach for: strict home-project prioritisation (tiering home rows above foreign ones).** If you widen precisely because the answer is not in your project, and home rows always win, then with `k = 8` (`DEFAULT_SEARCH_LIMIT`) and eight mediocre home matches you never see the foreign answer. The feature silently defeats itself — and silently is the operative word: the caller gets a full page of plausible rows and no signal that the widening contributed nothing. A full page is already not evidence of relevance (`mcp-api/spec.md:487` requires the description to say so), and tiering would make that worse in exactly the case the argument was passed for.
...
- **Not `scope`.** `mcp-api/spec.md:2723`'s scenario states "no property name SHALL be `scope` or `include_global`", and that requirement's subject is retirement — a name that used to mean something else is where the next silent misinterpretation goes.
...
The measured cost makes this concrete rather than stylistic: at 50 000 vectors over 8 projects the dense branch goes from ≈4.2 ms to ≈34.4 ms (≈8×, reproduced across four runs) (`measurements/vec-partition-capability.md` §4). Per explicit question that is fine. Per session start it is not.
...
Reclaiming the first gives **100** characters; reclaiming both gives **194**. A minimal conforming replacement — naming the argument, the restraint condition and the marker — measures ≈170 characters in draft, so **the second reclaim is probably required and its cost is real**: the entity-kind list is duplicated in substance inside the `entity` property's own uncapped `describe()`, but `mcp-api/spec.md:462` records that some clients do not surface per-argument descriptions, so moving it there is a partial loss rather than a free move.
...
**OQ4 — Is the phase-1 `Scope` collapse released with this change, or on its own?** Recorded default: **with this change**, as phase 1. It is a pure-refactor diff with no behavioural argument of its own, and D1's reasoning against splitting applies. Left open because an owner who wants the widening reviewed on a small diff could land phase 1 as a separate PR on the same branch and merge it first — which costs nothing but a second review and would make the widening diff dramatically easier to read.

**OQ5 — Should the eval corpus's 10 global memories become a third project, or be redistributed into the two existing ones?** Recorded default: **a third project**, because it is the cheapest source of the cross-project distractors phase 3 needs and because those 10 memories are cross-cutting conventions by content, which is what a shared project is. Left open because a third project changes every committed baseline's denominator, and an owner may prefer to take that hit once, in this change, rather than have it attributed to the ranking work.

**OQ6 — What is the correct `foreignScopeRate` denominator when a query legitimately widens?** For a non-widened query the cap is 0 and the metric is unambiguous. For a widened query, foreign rows are the _point_, so the metric must either be scored only over non-widened queries (simple, but then widened queries carry no gate at all) or be scored against an expected range per query (precise, but it is a hand-labelled expectation and therefore a fixture that can be wrong). Recorded default: **scored over non-widened queries only, as a hard 0 cap**, with widened queries gated by the ordinary `FLOOR_METRICS` against gold that deliberately lives in a foreign project. Named as the one genuinely undecided piece of phase 3's design.

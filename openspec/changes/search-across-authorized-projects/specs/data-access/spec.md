...
**The END-TO-END cost is much smaller than that ratio, and it is the figure any user-facing claim SHALL quote.** Measured through the search entry point on a realistically skewed corpus — one project holding 60% of it, the thinnest 2% — widening to every authorized project costs **1.32–1.35×** from the dominant project at 1 000 / 20 000 / 50 000 memories, and **2.39–2.55×** from the thinnest at 20 000 / 50 000 (5.0× at 1 000, where the fixed per-call costs still dominate the narrow arm). The gap between this and the statement ratio is not noise: only the dense and lexical reads scale with the widened set, while the query embedding, fusion, term statistics, relevance gate, ranking boost and row hydration do not. **The two SHALL NOT be presented in one table**, and a statement ratio SHALL NOT be quoted as what a caller waits for. The end-to-end figures above were taken against a prototype of the widened read whose returned ids were byte-identical to the shipped narrow read on every compared page; the shipped implementation SHALL be re-measured with the same instrument rather than inheriting them.

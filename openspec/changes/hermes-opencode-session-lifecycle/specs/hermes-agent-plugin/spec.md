...
**Reason for removal**: superseded by "Slug resolution cascade order" below. The precedence order is inverting (`.rembric` now wins over the env var, not the other way around) — the old title's core scenario ("Env wins over .rembric file") has no honest equivalent under the new order, so this requirement is replaced rather than edited in place.

...
- **Split *returned* from *used*.** `memory.search` returning a row in a page of eight is not evidence that the row was useful. Dereferencing it via `memory.get` is much closer. The search-path touch is narrowed, so broad triage scans stop inflating the recency signal — the same reasoning already applied deliberately to the HTTP passive-recall path.
- **Fix the touch-set bug found alongside it.** `search` touches the ids it *retrieved*, not the ids it *returned*: rows dropped by the live-status re-check get their `last_seen_at` bumped despite never reaching the caller.

...
describe('AgentSessionsService', () => {
  ...
  describe('truncateTitle helper', () => {
    it('returns input unchanged when within the cap', async () => {
      const { truncateTitle } = await import('@rembric/core');
  ...
  describe('computePlaceholderTitle — the insert paths do not reach a title validator', () => {
    ...
    it('keeps the clock suffix and cuts the basename when the cwd is deep', async () => {
      const { computePlaceholderTitle } = await import('@rembric/core');
  ...
  describe('NUDGE_FLOOR_MS — the one floor constant in the domain tree', () => {
    it('no other `_FLOOR_MS` constant is defined anywhere under apps/server/src or packages/core/src', async () => {
      ...
      // Both trees: the module that declares the floor moved to the domain
      // package, and a second declaration must fail where it now lives too.
      // Five levels up is the REPOSITORY root — `../../../..` resolves to
      // `apps/`, where `apps/server/src` is not a path and `git grep` finds
      // nothing at all (it exits non-zero, which threw before this fix).
      const out = execSync(

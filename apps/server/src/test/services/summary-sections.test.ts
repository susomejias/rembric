...
import { hasAnyHeading, mergeSummarySections, parseSummarySections } from '@rembric/core';
import { describe, expect, it } from 'vitest';

// The summary-section parser moved to the domain package, so the sibling read
// this guard performs is no longer a sibling.
const DOMAIN_SERVICES_DIR = new URL('../../../../../packages/core/src/services/', import.meta.url);
...
describe('purity', () => {
  it('imports no Date, repository or db module, and calls no Date constructor', () => {
    const src = readFileSync(
      fileURLToPath(new URL('summary-sections.ts', DOMAIN_SERVICES_DIR)),
      'utf8',
    );

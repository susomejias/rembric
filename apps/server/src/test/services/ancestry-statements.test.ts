import { MemoryService, PREDECESSOR_CAP } from '@rembric/core';
import { DISMISSAL_ANCESTRY_CAP, findSaveTimeCandidates } from '@rembric/core';
...
// The save-time candidate path moved to the domain package, so the sibling read
// this guard performs is no longer a sibling.
const DOMAIN_SAVE_TIME_CANDIDATES = new URL(
  '../../../../../packages/core/src/services/save-time-candidates.ts',
  import.meta.url,
);
...
describe('the two bounds are decoupled', () => {
  ...
  it('nothing outside memory.ts reads PREDECESSOR_CAP for suppression', async () => {
    ...
    const src = readFileSync(DOMAIN_SAVE_TIME_CANDIDATES, 'utf8');

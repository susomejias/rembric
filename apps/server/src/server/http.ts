...
async function handleMcpRequest(
  ...
  // refused before anything is constructed — the id predates a restart
  // (the transport registry is in-process only). `initialize` is exempt:
  // it establishes a fresh session regardless of what stale id it happens
  // to carry. Mirrors the JSON-RPC shape the SDK transport itself emits
  // for a session id that does not match its own.

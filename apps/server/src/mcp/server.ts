...
export function createMcpServer(opts: CreateMcpServerOptions): McpServer {
  ...
  registerTool(
    ...
    {
      description: `Save the end-of-session summary AND a short title. Call this at the END OF EVERY TURN that did real work — never end a working turn silent; do NOT wait for the literal word "done"/"listo". This call REPLACES the stored summary: send the session's CURRENT COMPLETE state, concise rather than exhaustive, not only what changed this turn. Put the current state FIRST — memory.context shows only the beginning of the stored summary, and only memory.session_get returns it in full. Can't see your earlier work? Call memory.session_get first, then write the whole updated state. Args: { summary (<=${SUMMARY_MAX_CHARS} chars, server rejects longer with invalid_input), title? (<=100 chars, descriptive of work done, NOT the cwd), sessionId? (pass it if you know your current session id — never invent one — to guarantee correct attachment when multiple sessions could be active) }. Body: ${SUMMARY_SECTIONS}. Does NOT end the session — use memory.session_end for that.`,

...
// Every session id it mints is `stub-session-<n>`; "recovers" 404s only
// session 1's tools/call (the one the extension holds at the moment the
// server evicted it), so the retry — carrying the freshly re-initialized
// session 2 — succeeds. "always404"/"always401" answer every tools/call the
// same way regardless of session, to prove the extension does not loop.
...
async function startRecoveryStub(
  behavior: 'recovers' | 'always404' | 'always401',
): Promise<RecoveryStub> {
  ...
  const server = createHttpServer((req, res) => {
    ...
    req.on('end', () => {
      ...
      if (message.method === 'initialize') {
        ...
        res.writeHead(200, {
          'Content-Type': 'application/json',
          'mcp-session-id': `stub-session-${sessionCounter}`,
        });
        res.end(
          JSON.stringify({
            jsonrpc: '2.0',
            ...
            result: {
              protocolVersion: '2025-06-18',
              capabilities: {},
              serverInfo: { name: 'recovery-stub', version: '0' },
            },
          }),
        );
        return;
      ...
      if (message.method === 'tools/call') {
        ...
        const isStale = behavior === 'always404' || sessionCounter === 1;
        if (isStale) {
...
describe('404 session-eviction recovery (server-driven)', () => {
  async function withStub(
    behavior: 'recovers' | 'always404' | 'always401',
    run: (stub: RecoveryStub, harness: Harness) => Promise<void>,
  ): Promise<void> {
    const stub = await startRecoveryStub(behavior);
    const url = process.env.REMBRIC_SERVER_URL;
    process.env.REMBRIC_SERVER_URL = stub.url;
    try {
      const harness = await startedHarness(`pi-recovery-${behavior}`);
      await run(stub, harness);
    } finally {
      process.env.REMBRIC_SERVER_URL = url;
      await stub.close();
    }
  ...
  it('a 404 on a tool call re-initializes exactly once and the retry succeeds', async () => {
    await withStub('recovers', async (stub, harness) => {
      const tool = toolNamed(harness, 'probe.tool');
      const outcome = await callThroughExtension(tool, {});

      expect(outcome.refused).toBe(false);
      expect(outcome.text).toBe('recovered');
      expect(stub.initializeCount()).toBe(2);
      expect(stub.toolCallCount()).toBe(2);
    });
  });

  it('a second 404 (on the retry) surfaces as an error with no third attempt', async () => {
    await withStub('always404', async (stub, harness) => {
      const tool = toolNamed(harness, 'probe.tool');
      const outcome = await callThroughExtension(tool, {});
  ...
  it('a 401 triggers no re-initialize and no retry', async () => {
    await withStub('always401', async (stub, harness) => {
      const tool = toolNamed(harness, 'probe.tool');
      const outcome = await callThroughExtension(tool, {});

      expect(outcome.refused).toBe(true);
      expect(stub.initializeCount()).toBe(1);
      expect(stub.toolCallCount()).toBe(1);
    });
  });

  it('recovery does not re-register tools', async () => {
    await withStub('recovers', async (stub, harness) => {
      const toolsBefore = harness.tools.length;
      const tool = toolNamed(harness, 'probe.tool');
      await callThroughExtension(tool, {});

      expect(harness.tools).toHaveLength(toolsBefore);
      expect(stub.toolsListCount()).toBe(1);
    });
  });
});

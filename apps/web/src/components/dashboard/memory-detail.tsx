...
/**
 * The memory detail hub: the review banner plus the
 * Overview/Lineage/Judgments tabs and the three tables behind them.
 *
 * One body, two carriers. `/dashboard/memories/[id]` renders it as a page, for a
 * direct link; the intercepted `@modal/(.)memories/[id]` route renders the same
 * body inside a `Sheet`, so a row click leaves the list visible behind the panel
 * and the two surfaces cannot drift apart. `loadMemoryDetail` is the single read
 * behind both, which is also why the sheet's header and the page's `ViewHead`
 * show one title from one query rather than two.
 *
...
/**
 * Every read the detail body needs, in one place. `null` when the id names no
 * row, which is what both callers turn into `notFound()`.
 */
export function loadMemoryDetail(id: string) {
...
export type MemoryDetailData = NonNullable<ReturnType<typeof loadMemoryDetail>>;

export function MemoryDetail({ data }: { data: MemoryDetailData }) {
  const {
    row,
    projectLabel,
    predecessors,

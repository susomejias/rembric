...
const SERVER_MANIFEST = join('apps', 'server', 'package.json');
...
/**
 * The running release identity — read from `apps/server`'s manifest at module
 * load, never written down here.
 *
 * `apps/server/package.json` IS the server release identity: release-please owns
 * the `server` component over `apps/server`, tags it `server-v<version>`, and
 * bumps this one field, and `apps/server/src/version.ts` reads the same field
 * against the same manifest. Spelling the release out in this file would make the
 * brand lie from the next release onwards — the version it prints is the version
 * that is running.
 *
 * The file is read rather than imported because the running app lives in
 * `apps/web`, and the server app is not part of this app's module graph. The
 * repository root is found by walking up from the working directory, which is
 * the app directory in both layouts (`<root>/apps/web` in a checkout, `/app/apps/web`
 * in the standalone image), so no absolute path is baked in.
 *
 ...
 *
 * `'0.0.0'` is the sentinel `apps/server/src/version.ts` also falls back to. It
 * cannot be mistaken for a release, which is the point: an unresolved identity
 * must not read as a plausible one.
 */
export const REMBRIC_VERSION = resolveVersion();

function resolveVersion(): string {
  const manifestVersion = readServerManifestVersion();
...
function readServerManifestVersion(): string | null {
  const root = findRepositoryRoot();
  if (root === null) return null;

  try {
    const pkg = JSON.parse(readFileSync(join(root, SERVER_MANIFEST), 'utf8')) as {
      version?: unknown;
    };
...
function findRepositoryRoot(): string | null {
  ...
  for (let depth = 0; depth < ROOT_SEARCH_DEPTH; depth++) {
    if (existsSync(join(dir, SERVER_MANIFEST))) return dir;

...
import { buildDoctorReportFactory, EMBEDDING_DIMS, type Embedder } from '@rembric/core';
...
/** `memory.doctor`'s payload for this fixture's handle, wired as the app layer wires it. */
function doctorReport(handle: DbHandle, dataDir: string) {
...
const CENSUS_COUNT_QUERIES: Record<string, string> = {
  ...
  dashboard_sessions: 'SELECT count(*) AS n FROM dashboard_sessions',
  ...
  memory_fts_vocab: 'SELECT count(*) AS n FROM memory_fts_vocab',
  memory_relations: 'SELECT count(*) AS n FROM memory_relations',
  memory_replaces: 'SELECT count(*) AS n FROM memory_replaces',
  memory_vec: 'SELECT count(*) AS n FROM memory_vec',
  oauth_authorization_codes: 'SELECT count(*) AS n FROM oauth_authorization_codes',
  oauth_clients: 'SELECT count(*) AS n FROM oauth_clients',
  oauth_tokens: 'SELECT count(*) AS n FROM oauth_tokens',
  prompts: 'SELECT count(*) AS n FROM prompts',
  prompts_fts: 'SELECT count(*) AS n FROM prompts_fts',
  sessions: 'SELECT count(*) AS n FROM sessions',
  token_projects: 'SELECT count(*) AS n FROM token_projects',
...
function census(handle: DbHandle): Census {
  ...
  const out: Census = {};
  for (const [table, query] of Object.entries(CENSUS_COUNT_QUERIES)) {
    if (!present.has(table)) continue;
    out[table] = handle.raw.prepare<[], { n: number }>(query).get()?.n ?? 0;
  }
  return out;
}
    ...
    const count = (
      ...
      raw
        .prepare<
          [],
          { v: number }
        >(view === 'memory' ? "SELECT count(*) AS v FROM memory WHERE scope = 'global'" : view === 'memory_entities' ? "SELECT count(*) AS v FROM memory_entities WHERE scope = 'global'" : view === 'memory_vec' ? "SELECT count(*) AS v FROM memory_vec WHERE partition_key = '__global__'" : "SELECT count(*) AS v FROM sqlite_temp_master WHERE name = '_vec_repartition'")
        .get()?.v ?? 0;
    const events: {
      line: string;
      ...
      const result = migrate(raw, {
        ...
        onProgress: (line) => {
          events.push({
            ...
            globalMemory: count('memory'),
            globalEntities: count('memory_entities'),
            stashExists: count('sqlite_temp_master'),
            globalVec: count('memory_vec'),
          });
        },
      });

...
import { runWithToolCallId } from '@rembric/core';
import { type Repositories } from '@rembric/db';
import { z, type ZodObject, type ZodRawShape } from 'zod';

import { aboutOutput, createAboutHandler } from './about-tool.js';
import type { LogInternalError } from './errors.js';
...
export function createMcpServer(opts: CreateMcpServerOptions): McpServer {
  ...
  const memoryHandlers = buildMemoryHandlers({
    ...
    getServer: () => server,
    logInternalError: opts.logInternalError,
  });
  registerTool(
    'memory.save',
    {
...
${SUMMARY_SECTIONS}
\`## Unfinished+why\` = what was still open when this session ended, and why — not a live TODO.

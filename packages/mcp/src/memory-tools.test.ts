...
beforeEach(() => {
  ...
  handlers = buildMemoryHandlers({ logInternalError, memory, projects });
...
describe('memory.save — strict path scoping', () => {
  ...
  it("rejects a save into an archived project with code 'project_archived'", async () => {
    const guarded = buildMemoryHandlers({ logInternalError, memory, projects });
...
describe('memory.save — entity extraction and linking (add-entity-index)', () => {
  it('extracts and links entities from title+content on save, independent of candidate detection', async () => {
    ...
    const entityHandlers = buildMemoryHandlers({ logInternalError, memory, repos, projects });
    const r = await runWithContext(fakeContext(projectA), () =>
      Promise.resolve(
        entityHandlers.save({
          type: 'project',
...
describe('memory.get / memory.search — entitiesTotal', () => {
  ...
  async function readAllThreeSurfaces(id: string) {
    const repos = createRepositories(db.handle.db);
    const h = buildMemoryHandlers({ logInternalError, memory, repos, projects });
...
describe('memory.search — relation expansion (include_relations)', () => {
  it('caps expansion at 5 entries even when more conflicts_with counterparts exist', async () => {
    ...
    const expandedHandlers = buildMemoryHandlers({ logInternalError, memory, relations, projects });
...
describe('memory.search / memory.get — relation annotations', () => {
  ...
  beforeEach(() => {
    ...
    relHandlers = buildMemoryHandlers({ logInternalError, memory, relations, projects });
...
describe('memory.* — router-activated project on an unscoped /mcp connection', () => {
  ...
  beforeEach(() => {
    ...
    routerHandlers = buildMemoryHandlers({ logInternalError, memory, router, projects });
  });
...
describe('memory.save — session attachment via HTTP-created sessions', () => {
  ...
  beforeEach(async () => {
    ...
    fallbackHandlers = buildMemoryHandlers({ logInternalError, memory, projects, agentSessions });
  });
  ...
  it('SessionRouter entry takes precedence over the DB fallback', async () => {
    ...
    const handlersWithRouter = buildMemoryHandlers({
      logInternalError,
      memory,
      projects,
      agentSessions,
...
describe('memory.confirm — session attachment (fix-audited-defects)', () => {
  ...
  beforeEach(async () => {
    ...
    confirmHandlers = buildMemoryHandlers({ logInternalError, memory, projects, agentSessions });
...
describe('memory.search — no argument widens the resolved scope', () => {
  ...
  beforeEach(() => {
    ...
    router = new SessionRouter();
    gateHandlers = buildMemoryHandlers({ logInternalError, memory, router, projects, repos });
    memory.save(
      {
        type: 'user',
